...
router.get("/users", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const usersWithStats = users.map(user => {
            const dashboard = dashboards.find(d => d.userId.toString() === user._id.toString());
            return {
                id: user._id,
        ...
        const contactsWithUploader = await Promise.all(
            contacts.map(async (contact) => {
                ...
                if (contact.uploadedBy) {
                    const uploader = await User.findById(contact.uploadedBy).select('name email');
...
router.get("/stats", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const totalPoints = dashboards.reduce((sum, dashboard) => sum + dashboard.availablePoints, 0);
...
router.delete("/contacts/:id", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        if (contact.uploadedBy) {
            uploaderDashboard = await Dashboard.findOne({ userId: contact.uploadedBy });
        }
            ...
            updatePromises.push(
                Dashboard.findByIdAndUpdate(
                    dashboard._id,
                    {
                        ...
                        $push: {
                            recentActivity: {
                                $each: [`Refunded 20 points: Contact "${contact.name || 'Unknown'}" was deleted by admin`],
                                $slice: -20
                            }
                        },
                        updatedAt: new Date()
            ...
            updatePromises.push(
                Dashboard.findByIdAndUpdate(
                    uploaderDashboard._id,
                    {
                        ...
                        $push: {
                            recentActivity: {
                                $each: [`Deducted 10 points: Your contact "${contact.name || 'Unknown'}" was deleted by admin`],
                                $slice: -20
                            }
                        },
                        updatedAt: new Date()
